    ...
    <script type="module">
        ...
        function WerktijdenTab({ werkrooster, statusBericht, rechten, onSave }) {
            ...
            const determineWorkDayType = (startTime, endTime) => {
                if (!startTime || !endTime) return 'ZV'; // Zittingsvrij (free day)
                ...
                const fullDayStart = 10 * 60; // 10:00
                const fullDayEnd = 15 * 60; // 15:00
                
                // Check for full working day (VVD)
                if (startMinutes < fullDayStart && endMinutes > fullDayEnd) {
                    return 'VVD'; // Vaste Vrije Dag (Full working day)
                }
                
                // Check if worked in morning hours (before 13:00)
                const workedMorning = startMinutes <= morningEnd;
                
                // Check if worked in afternoon hours (after 13:00)
                const workedAfternoon = endMinutes > afternoonStart;
                
                if (workedMorning && !workedAfternoon) {
                    return 'VVM'; // Vaste Vrije Middag (Free afternoon)
                }
                
                if (!workedMorning && workedAfternoon) {
                    return 'VVO'; // Vaste Vrije Ochtend (Free morning)
                }
                
                if (workedMorning && workedAfternoon) {
                    return 'VVD'; // Full working day
                }
                
                return 'ZV'; // Default to Zittingsvrij
            };
            ...
            const renderWerkdagenTabel = () => {
                if (!werkrooster && !bewerkModus) {
                    return h('tr', null,
                        h('td', { colSpan: 4, className: 'py-6 text-center text-gray-500 italic' },
                ...
                return DAGEN_VAN_DE_WEEK_DISPLAY.map(dag => {
                    ...
                    const dayType = determineWorkDayType(startTijd, eindTijd);
                        ...
                        h('td', { className: 'py-3' },
                            bewerkModus ? 
                                h('div', { className: 'flex gap-2 items-center' },
                                    h('input', {
                                        type: 'time',
                                        className: 'form-input text-sm w-24', // Changed from w-20 to w-24
                                        value: startTijd || '',
                                        onChange: (e) => setFormData(prev => ({ 
                                            ...prev, 
                                            [`${dagCapitalized}Start`]: e.target.value 
                                        }))
                                    }),
                                    ...
                                    h('input', {
                                        ...
                                        onChange: (e) => setFormData(prev => ({ 
                                            ...prev, 
                                            [`${dagCapitalized}Eind`]: e.target.value 
                                        }))
                                    })
                                ) :
                                (startTijd && eindTijd 
            ...
            const handleSave = async () => {
                try {
                    ...
                    DAGEN_VAN_DE_WEEK_DISPLAY.forEach(dag => {
                        ...
                        // Auto-set the day type based on work times
                        updatedFormData[`${dagCapitalized}Soort`] = determineWorkDayType(startTijd, eindTijd);
                    });
                ...
                h('div', { className: 'mb-4 p-3 bg-gray-50 rounded-lg' },
                    ...
                    h('div', { className: 'flex flex-wrap gap-2 text-xs' },
                        ...
                        h('span', { className: 'inline-flex items-center px-2 py-1 rounded-full bg-blue-100 text-blue-800' },
                            ...
                            'Vrije ochtend (alleen middag gewerkt)'
                        ),
                        h('span', { className: 'inline-flex items-center px-2 py-1 rounded-full bg-yellow-100 text-yellow-800' },
                            ...
                            'Vrije middag (alleen ochtend gewerkt)'
                        ),
