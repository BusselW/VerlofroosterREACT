...
export const MedewerkerForm = ({ onSave, onCancel, initialData = {}, title }) => {
    ...
    useEffect(() => {
        const loadOptions = async () => {
            try {
                // Test SharePoint connection first
                const connectionTest = await testSharePointConnection();
                console.log('SharePoint connection test:', connectionTest);
    ...
    const handleAutocompleteSelect = (user) => {
        ...
        // Extract username and ensure domain\username format
        let username = user.LoginName;
        if (username.includes('|')) {
            username = username.split('|').pop(); // Remove claims prefix if present
        }
        
        // Ensure proper domain\username format (not double backslashes)
        if (!username.includes('\\')) {
            // If no domain, this might be an external user or different format
            console.warn('Username does not contain domain separator:', username);
        }
        
        console.log('Processed username:', username); // Debug the final username format
