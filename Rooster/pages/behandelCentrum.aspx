            ...
            try {
                ...
                try {
                    ...
                    if (gebruikerResponse.ok) {
                        ...
                        const currentUserName = gebruikerData.d.Title;
                        const currentUserLoginName = gebruikerData.d.LoginName;
                        
                        // Update huidige gebruiker weergave
                        document.getElementById('huidige-gebruiker').textContent = currentUserName;
                        console.log("Gebruiker geladen:", currentUserName);
                        
                        // Haal teamleider informatie op en toon dit
                        try {
                            // Extract username from domain format (domain\username)
                            const username = currentUserLoginName.split('\\').pop();
                            
                            // Gebruik LinkInfo om teamleider op te halen
                            const teamLeader = await LinkInfo.getTeamLeaderForEmployee(username);
                            const teamLeaderElement = document.getElementById('team-leader-name');
                            
                            if (teamLeader) {
                                // Toon naam van teamleider
                                teamLeaderElement.textContent = teamLeader.Title || teamLeader.Naam || teamLeader.Username;
                                document.getElementById('team-leader-info').title = `Uw teamleider: ${teamLeader.Title || teamLeader.Naam}`;
                                console.log("Teamleider geladen:", teamLeader.Title);
                            } else {
                                teamLeaderElement.textContent = "Niet toegewezen";
                                document.getElementById('team-leader-info').title = "Geen teamleider toegewezen";
                            }
                        } catch (teamLeaderError) {
                            console.warn("Kon teamleider informatie niet laden:", teamLeaderError);
                            document.getElementById('team-leader-name').textContent = "Onbekend";
                        }
                    }
                } catch (userError) {
